...
class calcState {
    HashMap<String, BigInteger> vars;
...
public class Main {
        ...
        if (assignment.length == 2) {
            if (isIdentifier(assignment[0])) {
                if (isNumber(assignment[1])) {
                    state.vars.put(assignment[0], new BigInteger(assignment[1]));
    ...
    private static void calculateRPN(calcState state, ArrayList<String> list) {
        Stack<BigInteger> rpn = new Stack<>();
        for (String str : list) {
            if (isNumber(str)) {
                rpn.push(new BigInteger(str));
    ...
    private static BigInteger calc(String op, BigInteger arg2, BigInteger arg1) {
        if ("+".equals(op)) {
        ...
        if ("^".equals(op)) {
            ...
            while (j.compareTo(arg2) < 0) {
                i = i.multiply(arg1);
                j = j.add(BigInteger.ZERO);
            }
            return i;
        }
